...
# ================= 实验A的全新损失函数 =================
def total_loss(pred, mask, edge=None):
    """
    实验 A: BCE + IoU + Dice (无边缘加权)
    """
...
def evaluate_best_model(fold, val_list_path, image_root, gt_root, ckpt_path):
    eval_logger.info(f"\n--- 正在评估 Fold {fold} 最佳消融模型 (Dice Only) ---")
    ...
    eval_logger.info(f"Fold {fold} Ablation A (Dice) -> MAE: {avg[0]:.4f}, IoU: {avg[1]:.4f}, F1: {avg[2]:.4f}, SM: {avg[3]:.4f}")
...
def finetune_fold(fold):
    ...
    if os.path.exists(PRETRAINED_WEIGHTS):
        ...
        train_logger.info(f"✅ Ablation A Fold {fold}: 成功加载合成域预训练权重")
    else:
    ...
    train_logger.info(f"\n========== 启动 Ablation A (Dice Only) | Fold {fold} ==========")
    ...
    for epoch in range(epoch_num):
        ...
        pbar = tqdm(train_loader, desc=f"Ablation A Ep {epoch}", leave=False)
        for data in pbar:
            images = data['image'].cuda()
            gts = data['label'].cuda()
            edges = data['edge'].cuda() # 提取 edge (当前未加权，但保持接口一致)
        ...
        if (epoch+1) >= epoch_val:
            ...
            if f1 > best_f1:
                ...
                best_iou = iou
                best_epoch = epoch
                best_ckpt = os.path.join(CKPT_DIR, f"WPFormer_ablationA_fold{fold}_best.pth")
                torch.save(net.state_dict(), best_ckpt)
                status += " [★ 当前最优]"
            train_logger.info(status)
        net.train()
    
    train_logger.info(f"🏆 Ablation A Fold {fold} 结束 | Best Epoch: {best_epoch:02d} | Best F1: {best_f1:.4f} | Best IoU: {best_iou:.4f}\n")
...
if __name__ == '__main__':
    ...
    for f in range(1, 2):
        ...
        eval_logger.info(f"=> Fold 1 Ablation A (Dice Only) | 最终 IoU: {iou_score:.4f} | F1-Score: {f1_score:.4f}")