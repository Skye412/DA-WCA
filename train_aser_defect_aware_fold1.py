"""
=============================================================================
Experiment: ASER-lite + Defect-Aware Ambiguity (Fold 1)
Objective: 去除 Centerline 的干扰，专注于利用 GT 中的真实非裂缝缺陷 (0<val<255) 
           去教导 Ambiguity Head 抑制假阳性，冲刺更高的 Precision 和 F1。
=============================================================================
"""
...
# ================= 1. 配置区 =================
BASE_DIR = "/home/skye/data/Skye/DA-WCA/save/stage3_patch_aser_defect_aware_fold1"
...
def train_defect_aware_fold1():
    ...
    train_logger.info(f"\n{'='*20} 启动 Defect-Aware Ambiguity Fold 1 训练 {'='*20}")
    ...
    for epoch in range(epoch_num):
        ...
        for data in pbar:
            ...
            other_defects = data['other_defect'].cuda() # <--- 获取真实易混淆缺陷
            
            optimizer.zero_grad()
            
            # 注意：此处没有 centerline_logits，确保你的 WPFormer 是标准版
            preds, edge_logits, ambiguity_logits = net(images, return_aux=True)
            
            # 1. 主分割 Loss (依然不看 other_defects，保持主线纯洁)
            ...
            # 3. 【核心改造】 Defect-Aware Ambiguity Loss
            dilated = F.max_pool2d(clean_gt, kernel_size=5, stride=1, padding=2)
            ring_ambiguity = (dilated - clean_gt).clamp(0, 1)
            
            # Target 融合：几何膨胀环 + 真实非裂缝噪点
            ambiguity_gt = torch.clamp(ring_ambiguity + other_defects, 0, 1)
            # Mask 融合：主分割的有效区域 + 真实非裂缝噪点
            amb_sup_mask = torch.clamp(valid_mask + other_defects, 0, 1)
            
            amb_bce = F.binary_cross_entropy_with_logits(ambiguity_logits, ambiguity_gt, reduction='none')
            loss_amb = (amb_bce * amb_sup_mask).sum() / (amb_sup_mask.sum() + 1e-8)
        ...
        train_logger.info(f"Ep {epoch:02d} | Tot:{run_loss_tot/num_batches:.3f} | Seg:{run_loss_seg/num_batches:.3f} | Edg:{run_loss_edge/num_batches:.3f} | Amb:{run_loss_amb/num_batches:.3f}")
        ...
        if (epoch+1) >= epoch_val:
            ...
            if f1 > best_f1:
                best_f1 = f1
                best_ckpt = os.path.join(CKPT_DIR, f"WPFormer_defect_aware_fold{fold}_best.pth")
