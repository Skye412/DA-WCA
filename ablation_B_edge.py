...
# ================= 实验B的全新损失函数 =================
def total_loss(pred, mask, edge):
    """
    实验 B: Edge-Weighted BCE + IoU + Dice
    通过 edge_tensor 对裂缝边缘施加 3 倍惩罚，强迫抹平锯齿
    """
    ...
    # 核心：边缘权重加成
    edge_weight = 1.0 + 3.0 * edge
    norm_factor = (valid_mask * edge_weight).sum() + 1e-8
    bce = (bce * valid_mask * edge_weight).sum() / norm_factor

    # 2. IoU Loss
    inter = (pred * clean_mask * valid_mask).sum(dim=(2, 3))
    union = ((pred + clean_mask) * valid_mask).sum(dim=(2, 3))
    iou_loss = 1 - inter / (union - inter + 1e-8)
    
    # 3. Dice Loss
    dice_inter = (pred * clean_mask * valid_mask).sum(dim=(2, 3))
    dice_union = (pred * valid_mask).sum(dim=(2, 3)) + (clean_mask * valid_mask).sum(dim=(2, 3))
    dice_loss = 1 - (2.0 * dice_inter + 1e-8) / (dice_union + 1e-8)
    
    return bce + iou_loss.mean() + dice_loss.mean()
...
def evaluate_best_model(fold, val_list_path, image_root, gt_root, ckpt_path):
    eval_logger.info(f"\n--- 正在评估 Fold {fold} 最佳消融模型 (Edge + Dice) ---")
    ...
    results = []
    for filename in tqdm(filenames, desc=f"Evaluating Fold {fold}"):
        img_path = os.path.join(image_root, filename)
        gt_path = os.path.join(gt_root, filename)
    ...
    eval_logger.info(f"Fold {fold} Ablation B (Edge+Dice) -> MAE: {avg[0]:.4f}, IoU: {avg[1]:.4f}, F1: {avg[2]:.4f}, SM: {avg[3]:.4f}")
...
def finetune_fold(fold):
    ...
    if os.path.exists(PRETRAINED_WEIGHTS):
        ...
        train_logger.info(f"✅ Ablation B Fold {fold}: 成功加载合成域预训练权重")
    else:
    ...
    train_logger.info(f"\n========== 启动 Ablation B (Edge+Dice) | Fold {fold} ==========")
    ...
    for epoch in range(epoch_num):
        ...
        pbar = tqdm(train_loader, desc=f"Ablation B Ep {epoch}", leave=False)
        for data in pbar:
            images = data['image'].cuda()
            gts = data['label'].cuda()
            edges = data['edge'].cuda() # 核心：传递真实边界给模型
            
            optimizer.zero_grad()
            preds = net(images)

            loss = sum(total_loss(p, gts, edges) for p in preds)
            loss.backward()
            optimizer.step()
        ...
        if (epoch+1) >= epoch_val:
            ...
            if f1 > best_f1:
                ...
                best_iou = iou
                best_epoch = epoch
                best_ckpt = os.path.join(CKPT_DIR, f"WPFormer_ablationB_fold{fold}_best.pth")
                torch.save(net.state_dict(), best_ckpt)
                status += " [★ 当前最优]"
            train_logger.info(status)
        net.train()
    
    train_logger.info(f"🏆 Ablation B Fold {fold} 结束 | Best Epoch: {best_epoch:02d} | Best F1: {best_f1:.4f} | Best IoU: {best_iou:.4f}\n")
...
if __name__ == '__main__':
    ...
    for f in range(1, 2):
        ...
        eval_logger.info(f"=> Fold 1 Ablation B (Edge+Dice) | 最终 IoU: {iou_score:.4f} | F1-Score: {f1_score:.4f}")