...
# ================= 目录初始化 (针对 512 分辨率的 Stage 1) =================
BASE_DIR = "./save/stage1_512"
...
def train(fold):
    ...
    train_logger.info(f"\n--- 启动 Fold {fold} 纯真实域训练 (Stage 1 | 512分辨率 | BS=8) ---")
    best_f1 = 0
    best_ckpt = ""
    ...
    for epoch in range(epoch_num):
        ...
        if (epoch+1) >= epoch_val:
            ...
            if f1 > best_f1:
                best_f1 = f1
                best_ckpt = os.path.join(CKPT_DIR, f"WPFormer_stage1_512_fold{fold}_best.pth")
...
def generate_results(fold, list_path, img_root, gt_root, ckpt):
    ...
    for filename in tqdm(filenames, desc="Saving Images"):
        ...
        comp = np.hstack((cv2.resize(raw_img, (W, H)), cv2.cvtColor(gt_bw, cv2.COLOR_GRAY2BGR), cv2.cvtColor(pred_bw, cv2.COLOR_GRAY2BGR)))
...
if __name__ == '__main__':
    ...
    for f in range(1, 6):
        f1 = train(f)
        all_f1s.append(f1)
    eval_logger.info(f"\n🎯 Stage 1 (512px) 最终 5-Fold 平均 F1: {np.mean(all_f1s):.4f}")