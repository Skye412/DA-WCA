...
# 修改后的 get_logger：支持追加模式并统一输出
def get_logger(filename):
    logger = logging.getLogger("train_logger") # 使用固定名称
...
# 预先定义好全局唯一的日志文件
LOG_FILE = os.path.join(BASE_DIR, "train.txt")
...
def train_one_fold(fold):
    ...
    for epoch in range(epoch_num):
        ...
        if (epoch+1) >= epoch_val:
            ...
            if f1 > best_f1:
                best_f1 = f1
                best_ckpt = os.path.join(ckpt_dir, f"WPFormer_patch_fold{fold}_best.pth")
...
if __name__ == '__main__':
    ...
    final_logger.info("\n" + "="*50)
    final_logger.info(f"🎯 5-Fold (Baseline) 汇总结果:")
    # 假设你 Fold 1 的结果是 0.6493，也可以手动加进列表
    # all_f1s.insert(0, 0.6493) 
